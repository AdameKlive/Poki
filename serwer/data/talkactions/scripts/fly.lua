...
function onSay(player, words)
    if words == "!up" and player:getStorageValue(storageDive) > 0 then
        ...
        if tile and isInArray(waterIds, tile:getGround():getId()) and hasSummons(player) and MonsterType(player:getSummon():getName()):isSurfable() > 0 then
            ...
            player:sendCancelMessage("Potrzebujesz Pokemona zdolnego do surfowania, aby móc się wynurzyć.")
        end
        return true
    end

    if not player:isOnFly() then
        player:sendCancelMessage("Nie latasz.")
        return false
    end
    ...
    if words == "!up" then
        ...
        if ret == false then
            player:sendCancelMessage("Nie możesz polecieć w górę.")
        else
            player:activateFly()
    ...
    elseif words == "!down" then
        ...
        if ret == false then
            player:sendCancelMessage("Nie możesz polecieć w dół.")
        else
