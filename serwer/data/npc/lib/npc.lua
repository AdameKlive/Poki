-- Włączenie Zaawansowanego Systemu NPC
...
function doNpcSellItem(cid, itemId, amount, subType, ignoreCap, inBackpacks, backpack)
	...
	for i = 1, amount do -- normalna metoda dla przedmiotów niestackowalnych
...
function doPlayerSellItem(cid, itemid, count, cost)
	...
	if doPlayerTakeItem(cid, itemid, count) then
		if not player:addMoney(cost) then
			error('Nie można dodać pieniędzy do ' .. player:getName() .. '(' .. cost .. 'gp)')
		end
		return true
	end
	return false
