	...
	if msgcontains(msg, 'key') then
		if player:getStorageValue(Storage.thievesGuild.Mission06) == 1 and player:getSex() == PLAYERSEX_FEMALE then
			...
			if headItem and headItem.itemid == 2665 and player:getStorageValue(Storage.postman.Rank) == 5 then
				...
				npcHandler:say('Och, wyglądasz wspaniale w swoim mundurze! Wy, arcy-listonoszki, jesteście nie tylko odważne, ale i piękne. Proszę, weź to, to klucz, o który prosiłaś. Tylko obiecaj, że będziesz mnie od czasu do czasu odwiedzać!', cid)
			end
		end
	end
	return true
