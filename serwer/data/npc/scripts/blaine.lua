local storage = quests.volcanoBadge.prizes[1].uid -- Identyfikator storage dla nagrody za Odznakę Wulkanu
local pokemons = 
{
	[1] = 
		{
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						...
						selfSay('To było niesamowite, ' .. player:getName() .. "! Twoje Pokemony zamieniły moje w proch. Zabierz ze sobą Insygnium Wulkanu!", cid)
