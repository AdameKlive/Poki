local exitPosition = Position(264, 1074, 7) -- Pozycja wyjścia z areny
local nextPosition = Position(206, 1071, 7) -- Pozycja po zwycięstwie
local pokemons = 
{
	[1] = 
		{
...
local function creatureGreetCallback(cid, message)
	...
	if npcHandler:hasFocus() then
		selfSay("Poczekaj na swoją kolej, " .. Player(cid):getName() .. "!")
		return false
	end
	return true
end
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') then
		local player = Player(cid)
		if player then
			selfSay('Pokażę ci, jak walczy prawdziwy trener!', cid)
			...
			if npcHandler:isFocused(cid) and npcHandler.topic[cid] == 1 then
				...
				if not monster then
					if pokemons[duelStatus] then
						selfSay(pokemons[duelStatus].name .. ", wybieram ciebie!")
						...
						selfSay(player:getName() .. ", wygrałeś! Wygląda na to, że masz ukryty talent. Nie mam już nic do powiedzenia. Idź teraz! Biegnij i wyzwij następnego członka Ligi Pokemon! Gwarantuję, że nie będzie łatwo.", cid)
						player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
